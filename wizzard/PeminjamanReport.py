...
class PeminjamanReport(models.TransientModel):
    _name = 'rokilibrary.peminjamanreport'
    ...
    def action_peminjaman_report(self):
        ...
        print(filter)
        peminjaman = self.env['rokilibrary.peminjaman'].search_read(filter)
        print(peminjaman)
        data = {
            'form': self.read()[0],
        ...
        print(data)
        return self.env.ref('rokilibrary.wizzard_peminjamanreport_pdf').report_action(self, data=data)
