...
class GenreBuku(models.Model):
    ...
    buku_ids = fields.One2many(comodel_name='rokilibrary.buku', inverse_name='genrebuku_id', string='Daftar Buku')
    ...
    def _compute_jml_item(self):
        for rec in self:
            a = self.env['rokilibrary.buku'].search([('genrebuku_id', '=', rec.id)]).mapped('name')
            b = len(a)
            rec.jml_item = b
            rec.daftar = a
