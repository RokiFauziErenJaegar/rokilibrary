...
class Peminjaman(models.Model):
    ...
    detailpeminjaman_ids = fields.One2many(comodel_name='rokilibrary.detailpeminjaman', inverse_name='peminjaman_id', string='Detail Peminjaman')
    ...
    def action_done(self):
        ...
        if self.detailpeminjaman_ids:
                a=[]
                for rec in self:
                    a = self.env['rokilibrary.detailpeminjaman'].search([('peminjaman_id','=',rec.id)])
                    print(a)
                for ob in a:
    ...
    def write(self,vals):
        ...
        for rec in self:
            b = self.env['rokilibrary.detailpeminjaman'].search([('peminjaman_id','=',rec.id)])
            print(b)
            for databaru in b:
                if databaru in a:
...
class DetailPeminjaman(models.Model):
    _name = 'rokilibrary.detailpeminjaman'
    _description = 'New Description'

    name = fields.Char(string='Nama')
    peminjaman_id = fields.Many2one(comodel_name='rokilibrary.peminjaman', string='Detail Peminjaman')
    buku_id = fields.Many2one(comodel_name='rokilibrary.buku', string='List Buku')
    ...
    def create(self,vals):
        ...
        if record.qty:
            self.env['rokilibrary.buku'].search([('id','=',record.buku_id.id)]).write({'stok' : record.buku_id.stok - record.qty})
        return record
